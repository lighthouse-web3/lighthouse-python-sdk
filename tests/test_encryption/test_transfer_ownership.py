...
class TestKavachTransferOwnership(unittest.TestCase):
    def test_transfer_ownership(self):
      ...
      result = Kavach.transferOwnership(
        address=public_key,
        cid = "QmbFMke1KXqnYyBBWxB74N4c5SBnJMVAiMNRcGu6x1AwQH",
        auth_token=f"0x{auth_token}",
        new_owner="0xF0Bc72fA04aea04d04b1fA80B359Adb566E1c8B1"
      )

      self.assertIsNone(result["error"])
      self.assertTrue(result["isSuccess"])

    def test_transfer_ownership_invalid_address(self):
      public_key = os.environ.get("PUBLIC_KEY")
